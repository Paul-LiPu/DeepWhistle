...
def evaluate_detection_network_lmdb_psnr(net, test_dataset, config, iterations, test_batch_num, test_pic_num):
    psnr = []
    for i_batch, sample_batched in enumerate(test_dataset):
        input = Variable(sample_batched['input']).type(dtype)
        output = net(input)
        output = np.clip((output.cpu().data.numpy()) * 255., 0, 255).astype(np.uint8)
        label = np.clip(sample_batched['label'].numpy() * 255., 0, 255).astype(np.uint8)

        if i_batch == test_batch_num:
            output_patch = test_pic_num
            output_image = np.clip(input.data.cpu().numpy()[output_patch, 0, :, :] * 255, 0, 255).astype(np.uint8)
            cv2.imwrite(config.test_folder + '/test_image_iter_' + str(iterations) + '_input.png', output_image)
            output_image = output[output_patch, 0, :, :]
...
def cal_PR(output, label):
    return []

def evaluate_detection_network_lmdb_PR(net, test_dataset, config):
    recall = np.asarray([])
    for i_batch, sample_batched in enumerate(test_dataset):
        input = Variable(sample_batched['input']).type(dtype)
        binary_criteria = 0.5
