    ...
    methods
        function tt = TonalTracker(Filename, Start_s, Stop_s, varargin)
            ...
            if exist([path, '/', filename, '/'])
                files = dir([path, '/', filename,'/','*predict.png']);
                temp = strsplit(files(1).name, '0_predict');
                file_basename = temp{1};
                count = 0;
                for i = 1 : length(files)
                    files(i).name = [file_basename, num2str(count), '_predict.png'];
                    count = count + 1;
                end
                tt.result_files = files;
                tt.result_path = [path, '/', filename];
            end
            ...
            while k <= length(varargin)
                switch varargin{k}
                    ...
                    case 'BlockLength'
                        tt.thr.blocklen_s = varargin{k+1};k=k+2;
                    case 'UsePropMap'
                        tt.usePropMap = varargin{k+1};k=k+2;
                    case 'ResultPath'
                        tt.result_path = varargin{k+1};k=k+2;
                        files = dir([tt.result_path, '/', '*predict.png']);
                        temp = strsplit(files(1).name, '0_predict');
                        file_basename = temp{1};
                        count = 0;
                        for i = 1 : length(files)
        ...
        function startBlock_lipu(tt)
            ...
            tt.snr_power_dB = im2double(imread([tt.result_path, '/', tt.result_files(tt.block_idx).name]));
            tt.snr_power_dB = tt.snr_power_dB(end:-1:1, :);
%             figure();imshow(tt.snr_power_dB);
            
            if (tt.block_idx == 1)
                timeidx = tt.Indices.timeidx;
%                 temp = find(timeidx >= tt.Start_s, 1);
                tt.frame_idx = max(find(timeidx >= tt.Start_s, 1) - 1, 0);
            else
                tt.frame_idx = 0;
            end
%             tt.advanceFrameInBlock();
        ...
        function advanceFrameInBlock(tt)
            ...
            if tt.frame_idx <= length(tt.Indices.timeidx)
                tt.current_s = tt.Indices.timeidx(tt.frame_idx);
            else
                tt.current_s = tt.current_s + tt.thr.advance_ms / 1000;
            end
%             tt.current_s = tt.Indices.timeidx(tt.frame_idx);
            if (tt.callbackSet)
...
%         end
        ...
        function processFile_lipu(tt)
            tt.startBlock_lipu();
            while (tt.hasMoreFrames())
                tt.advanceFrame_lipu();
                tt.processCurrentFrame();
            end
            tt.finalizeTracking();
        end
